...
struct CalendarPicker: View {
    @Bindable var vm = CalendarPicker.ViewModel()
    ...
    var body: some View {
        ...
        VStack(spacing: 12){
            HStack (spacing: 0) {
                ...
                Button{
                    withAnimation(.spring(.bouncy)){
                        vm.isWeelView.toggle()}
                }label:{
                    Text(vm.formatter.string(from: vm.displayedMonth)).font(.headline)
                    Image(systemName: vm.isWeelView ?  "chevron.down" : "chevron.right")
                        .font(.footnote)
                        .bold()
                ...
                if !vm.isWeelView{
                    HStack{
                        ...
                        Button {
                            withAnimation(.interactiveSpring){vm.ChangeWeek(by: -1); vm.ChangeMonth(by: -1)}
                        } label: {
            ...
            if !vm.isWeelView {
                 ...
                 Divider().frame(height: 0.5).background(.divider)
            }
